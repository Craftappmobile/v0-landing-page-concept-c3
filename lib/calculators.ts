...
function createSeoCalculator(input: SeoCalculatorInput): CalculatorDefinition {
  return {
    slug: input.slug,
    title: `${input.shortTitle} онлайн — демо-розрахунок для в'язання`,
    shortTitle: input.shortTitle,
    description: `SEO demo page для теми «${input.topic}»: коротка формула, приклад розрахунку, практичні підказки й посилання на пов'язані калькулятори без 404.`,
    keywords: input.keywords,
    intro: `Це корисна демо-сторінка для запиту «${input.topic}». Вона прибирає 404 і дає базову логіку розрахунку: що виміряти, яку формулу застосувати та як перевірити результат перед в'язанням.`,
    demo: {
      heading: input.topic,
    ...
    faq: [
      {
        question: `Чи можна використовувати цю сторінку як повний калькулятор для теми «${input.topic}»?`,
        answer:
          "Це демо-сторінка з базовою формулою і прикладом. Для точного виробу потрібно додатково враховувати фасон, свободу облягання, візерунок, пряжу й вашу фактичну щільність після прання зразка.",
      },
      {
...
export const calculators: CalculatorDefinition[] = [
  {
    ...
    description:
      "Демо-калькулятор витрати пряжі: оберіть виріб, стать або категорію, введіть обхват чи вік дитини та отримайте орієнтовну витрату в метрах або грамах.",
  ...
  },
  {
    ...
    description:
      "Демо-калькулятор складання пряжі: перерахуйте метраж у кілька ниток, вагу основної та додаткової нитки, а також загальний метраж при поєднанні різних пряж.",
  ...
  },
  {
    ...
    description:
      "Демо-калькулятор щільності в'язання: введіть кількість петель і рядів у 10 см, щоб отримати петлі та ряди на 1 см для точних розрахунків виробу.",
  ...
  },
  {
    ...
    description:
      "Демо-калькулятор адаптації опису в'язання: перерахуйте кількість петель під свою щільність і визначте загальну витрату пряжі в метрах за даними майстер-класу.",
  ...
  },
  {
    ...
    description:
      "Демо-калькулятор реглан-погону: розрахуйте стартову горловину, розподіл на перед, спинку і погони, росток, точки розвороту та етапи прибавок до пройми.",
